# Администрирование кластера Kubernetes

**Языки:** [English](README.md) · [简体中文](README_zh.md) · [Español](README_es.md) · [Français](README_fr.md) · [Deutsch](README_de.md) · [日本語](README_ja.md) · [Русский](README_ru.md) · [한국어](README_ko.md) · [Português](README_pt.md)

<p align="center">
  <a href="https://labex.io/ru/courses/kubernetes-cluster-administration">
    <img src="https://course-cover.labex.io/kubernetes-cluster-administration.png?lang=ru" alt="Администрирование кластера Kubernetes">
  </a>
</p>

Изучите администрирование кластера Kubernetes. Этот модуль охватывает обслуживание узлов (Node Maintenance), пространства имен и квоты (Namespaces and Quotas), авторизацию RBAC (RBAC Authorization), секреты и ConfigMaps (Secrets and ConfigMaps), устранение неполадок рабочих нагрузок (Troubleshooting Workloads). Вы освоите эти важные навыки Linux посредством практических лабораторных работ и реальных задач.

[Начать курс на LabEx](https://labex.io/ru/courses/kubernetes-cluster-administration)

## Упражнения

|   Индекс | Название                              | Сложность   | Практика                                                                                                                                            |
|----------|---------------------------------------|-------------|-----------------------------------------------------------------------------------------------------------------------------------------------------|
|        1 | Обслуживание узлов                    | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/node-maintenance-636608?course=kubernetes-cluster-administration'>Начать лабораторию</a>          |
|        2 | Пространства имен и квоты             | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/namespaces-and-quotas-636607?course=kubernetes-cluster-administration'>Начать лабораторию</a>     |
|        3 | Авторизация RBAC                      | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/rbac-authorization-636609?course=kubernetes-cluster-administration'>Начать лабораторию</a>        |
|        4 | Secrets и ConfigMaps                  | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/secrets-and-configmaps-636610?course=kubernetes-cluster-administration'>Начать лабораторию</a>    |
|        5 | Устранение неполадок рабочих нагрузок | Начинающий  | <a target='_blank' href='https://labex.io/ru/labs/troubleshooting-workloads-636611?course=kubernetes-cluster-administration'>Начать лабораторию</a> |
|        6 | Миссия по спасению кластера           | Средний     | <a target='_blank' href='https://labex.io/ru/labs/cluster-rescue-mission-636763?course=kubernetes-cluster-administration'>Начать испытание</a>      |

## About LabEx

<div align="left"><p><a href="https://labex.io"><strong>LabEx</strong></a> is a <strong>hands-on learning platform for beginners</strong>.</p><p>Explore <a href="https://labex.io/learn/linux"><strong>Linux</strong></a>, <a href="https://labex.io/learn/devops"><strong>DevOps</strong></a>, <a href="https://labex.io/learn/cybersecurity"><strong>Cybersecurity</strong></a>, and <strong>more</strong> — all directly in your browser.</p><p>Learn step by step through <strong>interactive labs</strong>, <strong>guided exercises</strong>, and <strong>real-world projects</strong>. 🌱<br />No setup, no stress — just practice and grow your skills by doing.</p><br /><p><a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/assets/elements/badges/download-on-the-app-store.svg" alt="Download on the App Store" height="54" /></a>&nbsp;<a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/app-store/marketing/guidelines/images/badge-download-on-the-mac-app-store.svg" alt="Download on the Mac App Store" height="52" /></a></p><br /><p>📖 Need help? Visit our <a href="https://support.labex.io/">Help Center</a> or email info@labex.io</p></div>

